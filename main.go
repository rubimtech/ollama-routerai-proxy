...
func main() {
	...
	provider := NewRouterAIProvider(baseUrl, apiKey)
	...
	r.POST("/api/chat", func(c *gin.Context) {
		...
		if !streamRequested {
			...
			response, err := provider.Chat(request.Messages, fullModelName, request.Tools, request.ToolChoice)
			if err != nil {
		...
		stream, err := provider.ChatStream(request.Messages, fullModelName, request.Tools, request.ToolChoice)
		if err != nil {
		...
		if request.Stream {
			...
			for {
				...
				if len(choice.Delta.ToolCalls) > 0 {
					toolCalls := make([]map[string]interface{}, 0, len(choice.Delta.ToolCalls))
					for _, tc := range choice.Delta.ToolCalls {
						...
						if tc.Function.Name != "" {
							funcMap["name"] = tc.Function.Name
						}
						if tc.Function.Arguments != "" {
							funcMap["arguments"] = tc.Function.Arguments
						}
						if len(funcMap) > 0 {
							tcMap["function"] = funcMap
						}
